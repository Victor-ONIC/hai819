...
// /*
  ...
  phongIL = phongIL * 0.8 + specular + 0.3;
    ...
    if(fragBlockType == 1){
        ...
        texColor = texture(bedrock_tex, fragTexCoord);
        fragColor = vec4((phongIL * texColor.rgb), 1.0);
    }
    else if(fragBlockType == 5){
        texColor = texture(lava_tex, fragTexCoord);
        fragColor = vec4((phongIL * texColor.rgb), 1.0);
    }
    else if(fragBlockType == 6){
        texColor = texture(sand_tex, fragTexCoord);
        fragColor = vec4((phongIL * texColor.rgb), 1.0);
    }
